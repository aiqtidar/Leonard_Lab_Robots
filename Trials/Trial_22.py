...
def read_u():
    ...
    beta = [color_103*k_b-0.47, 0.45-color_105*k_r]
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if (U != [0,0]):
            ...
            print("Beta Change = " + str(beta_change))
            print("Beta Color = " + str(beta_color))


            
            for i in range(0,n_robots):

                #Check for massive changes in Beta after 3 seconds of running
                if (abs(start - time.time()) > 3) and (U[i]>1):
                    if (last_beta_color != beta_color):
                        beta_change = True

                if (abs(x_locations[1]) < 0.4) or (abs(x_locations[0]) < 0.4):
                    beta_change = False

                #Sets the direction to circle depending on the robot's y-location
                if circle_direction == True:
                    x_sign[i] = np.sign(x_locations[i])
                    if ((robot_locations[i].linear.y > 3.5)):
                        angular_sign[i] = -1
                    elif ((robot_locations[i].linear.y < 3.5)):
                        angular_sign[i] = 1
                ...
                elif ((U[i]<1) and (absol(x_locations[i]) < 0.21)):
                ...
                if (state[i] == 1):
                    ...
                    if (abs(rad_start - time.time()) < 3):
                        last_velocity_command[i].linear.x = -np.sign(x_sign[i])*0.25
                ...
                #Decision making
                    ...
                    #Set speeds to circle
                    last_velocity_command[i].linear.x = np.sign(x_sign[i])*0.20
                    last_velocity_command[i].angular.z = np.sign(angular_sign[i])*.7
                    pub_velocity_command[i].publish(last_velocity_command[i])
        ...
        with open('Data_Run_22.csv', 'a') as csvFile:
