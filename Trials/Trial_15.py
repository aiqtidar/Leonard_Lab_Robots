...
def read_u():
    ...
    if (float(beta[0])+ float(beta[1])) > 0.7:
        beta_color = 6
    elif (float(beta[0])+ float(beta[1])) < (-0.7):
        beta_color = 1
    else: beta_color = 0
    ...
    while not rospy.is_shutdown():
        ...
        if (U != [0,0]):
            ...
            for i in range(0,n_robots):
                ...
                if ((U[i]>1) and (state_change[i] == True)) or ((beta_change == True)):
                    #State to decide
                    state[i] = 2
                ...
                elif ((absol(x_locations[i]) > 1.9) and (U[i]>1)):
                ...
                if state[i] == 2:
                    if (state[i] != state[abs(1-i)]):
                        if (state[abs(1-i)]) == 3:
                            fake_robot_locations[abs(1-i)].linear.x = np.sign(x_locations[i])*2
                        elif (state[abs(1-i)]) == 1:
                            fake_robot_locations[abs(1-i)].linear.x = 0
