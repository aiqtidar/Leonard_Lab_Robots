...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(0,n_robots):
            ...
            #Decision making starts when start[i]==True, U[1]>1
            else:
                print(x_locations[i])
                ...
                if (abs(x_locations[i]) > 1.5) and (U[i]>1):
                    decision = True
                    number_decision = number_decision + 1
                elif (U[i]<1):
                    decision = False

                #If it has been, circle. Else reorient
                if (decision == True):
                    #Set speeds to circle
                    last_velocity_command[i].linear.x = -0.33
                    last_velocity_command[i].angular.z = .5
                ...
                #Gets to the center after making a decision
                if (number_decision>0) and (abs(x_locations[i]) < 0.1):
                    start[i] = True
